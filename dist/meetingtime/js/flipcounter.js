...
var flipCounter = function(d, options){
	 ...
	 */
	this.setUpdateCallback = function(n){
		updatecallback = n;
		return this;
	};
		...
		if (elems.length){
			for (var x in elems ){
				if (elems[x] && elems[x].className && elems[x].className == classname){
					elems[x].className = newclassname;
				}
			}
		}
	}
	 ...
	 *
	 * @param {bool} a
	 *   Should a dot be the seperator? , true or false
	 */
	this.setDot = function(a){
		if (a != o.dot) {
			o.dot = a;
			// change dots into commas or otherwise around
			changeDots(a);
		}
		return this;
	};
...
//wvw
		if(updatecallback) { updatecallback(o.value) }
	}
	...
	function addDigit(len, digit){
		...
		if (li % 3 == 0){
			...
			if (o.dot) {
				newComma.innerHTML = '<li class="dt"></li>';
			} else {
	...
	function removeDigit(id){
		...
		if (((" " + first.className + " ").indexOf(" s ") > -1 ) || ((" " + first.className + " ").indexOf(" dt ") > -1 ) ) {
